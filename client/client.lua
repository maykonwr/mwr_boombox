...
function LoadSpeakers()
    TriggerCallback('mwr_boombox:callback:getBoomboxs', function(result)
...
RegisterNUICallback('playSong', function(data)
    TriggerServerEvent('mwr_boombox:server:Playsong', data)
end)

RegisterNUICallback('getNewPlaylist', function(data, cb)
    TriggerCallback('mwr_boombox:callback:getNewPlaylist', function(result)
        cb(result)
    end, data)
end)

RegisterNetEvent('mwr_boombox:client:notify', function(msg)
...
RegisterNUICallback('changeDist', function(data)
    TriggerServerEvent('mwr_boombox:server:SyncNewDist', data)
end)

RegisterNUICallback('syncVolume', function(data)
    TriggerServerEvent('mwr_boombox:server:SyncNewVolume', data)
end)

RegisterNUICallback('deletePlayList', function(data)
    TriggerServerEvent('mwr_boombox:server:deletePlayList', data)
end)

RegisterNUICallback('importNewPlaylist', function(data)
    TriggerServerEvent('mwr_boombox:server:importNewPlaylist', data)
end)

RegisterNetEvent('mwr_boombox:client:updateBoombox', function(id, data)
    speakers[id] = data
...
RegisterNetEvent('mwr_boombox:client:updateVolume', function(id, vol)
    speakers[id].volume = vol
end)

RegisterNetEvent('mwr_boombox:client:updateDist', function(id, dist)
    speakers[id].maxDistance = dist
end)

RegisterNetEvent('mwr_boombox:client:doAnim', function()
...
RegisterNetEvent('mwr_boombox:client:insertSpeaker', function(data)
    table.insert(speakers, data)
    SendReactMessage('createRepro', data.url)
end)

RegisterNetEvent('mwr_boombox:client:deleteBoombox', function(id)
    speakers[id].permaDisabled = true
end)

RegisterNUICallback('prevSong', function(data)
    if data.repro == reproInUi then
        TriggerServerEvent('mwr_boombox:server:prevSong', data)
    end
end)

RegisterNUICallback('nextSong', function(data)
    if data.repro == reproInUi then
        TriggerServerEvent('mwr_boombox:server:nextSong', data)
    end
end)

RegisterNUICallback('pauseSong', function(data)
    if data.repro == reproInUi then
        TriggerServerEvent('mwr_boombox:server:pauseSong', data)
    end
end)

RegisterNUICallback('syncNewTime', function(data)
    if data.repro == reproInUi then
        TriggerServerEvent('mwr_boombox:server:syncNewTime', data)
    end
end)
    ...
    while true do
            ...
            if distance < v.maxDistance and not v.permaDisabled then
                ...
                if distance < 1.5 and not v.isMoving then
                    ...
                    if not isInUI and not movingASpeaker and TARGET_DELETE then
                        TARGET_DELETE = false
                        TriggerServerEvent('mwr_boombox:server:deleteBoombox', k, v.coords.x)
                        Wait(200)
                    end
                    if not movingASpeaker and TARGET_PICK_UP then
                        TARGET_PICK_UP = false
                        TriggerCallback('mwr_boombox:callback:canMove', function(canMove)
                            if canMove then
                                movingASpeaker = true
        ...
        if movingASpeaker then
            ...
            if IsControlJustPressed(1, Config.KeyToPlaceSpeaker) then
                ...
                TriggerServerEvent('mwr_boombox:server:updateObjectCoords', movingSpeakerId)
                movingSpeakerId = 0
...
RegisterNetEvent('mwr_boombox:client:updatePlayerMoving', function(id, src)
...
RegisterNUICallback('getPlaylists', function(_, cb)
    TriggerCallback('mwr_boombox:callback:getPlaylists', function(result)
        cb(result)
    end)
end)

RegisterNetEvent('mwr_boombox:client:syncLastCoords', function(id, coords)
    speakers[id].coords = coords
    speakers[id].isMoving = false
    speakers[id].playerMoving = -2
end)

RegisterNetEvent('mwr_boombox:client:syncLastCoordsSync', function(id, coords)
    speakers[id].coords = coords
end)

RegisterNUICallback('addSong', function(data)
    TriggerServerEvent('mwr_boombox:server:addSong', data)
end)

RegisterNUICallback('deleteSongPlaylist', function(data)
    TriggerServerEvent('mwr_boombox:server:deleteSongPlaylist', data)
end)

RegisterNetEvent('mwr_boombox:client:resyncPlaylists', function ()
    SendReactMessage('getPlaylists')
end)
...
exports.ox_target:addModel('prop_boombox_01', {
    {
        type = 'client',
